...
class YOLOV5_ONNX(object):
    def __init__(self, onnx_path, img_size=(640, 640), iou_threshold=0.5, conf_threshold=0.3):
        """初始化onnx"""
        self.onnx_session = onnxruntime.InferenceSession(onnx_path, providers=['CUDAExecutionProvider'])
        print(onnxruntime.get_device(), "  ", self.onnx_session .get_providers())
        self.input_name = self.get_input_name()
        self.output_name = self.get_output_name()
        self.classes = ['person', 'rider', 'vehicle', 'non_vehicle']
    ...
    def nms(self, prediction, conf_thres=0.1, iou_thres=0.6, agnostic=False):
        ...
        for xi, x in enumerate(prediction):  # image index, image inference
            ...
            # 同一个非机动车目标可以检出非机动车和人骑车， 过滤非机动车和人骑车重复框问题
            c[c[:] == 12288] = 4096
    ...
    def infer(self, src_img):
        ...
        results = self.nms(pred, self.conf_threshold, self.iou_threshold)

        # 映射到原始图像
        for det in results:  # detections per image
            if len(det):
                det[:, :4] = self.scale_coords(img.shape[2:], det[:, :4], src_size).round()

        return results

    def plot_one_box(self, x, img, color=None, label=None, line_thickness=None, style="dotted"):
        ...
        if style == "dotted":
            self.dotted_rectangle(img, c1, c2, color, thickness=tl)
        else:
        ...
        if label:
            ...
            cv2.putText(img, label, (c1[0], c1[1] + 18), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)

    def plot_one_region(self, region, img, color=(0, 0, 255), label=None, line_thickness=2):
        points = np.array(region, np.int32)
        img = cv2.polylines(img, [points], isClosed=True, color=color, thickness=line_thickness)
        draw_point = points[points[:, 0].argmin()]
        if label:
            cv2.putText(img, label, draw_point, 0, line_thickness, [0, 255, 255], thickness=line_thickness, lineType=cv2.LINE_AA)

    def draw(self, img, boxinfo, save_path):
    ...
    def dotted_rectangle(self, img, pt1, pt2, color, thickness=1, gap=20):
        points = [pt1, (pt2[0], pt1[1]), pt2, (pt1[0], pt2[1]), pt1]
